...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const loop = (time) => {
      ...
      if (!draggingRef.current && !inertiaRAF.current && !openingRef.current) {
        const step = (0.005 * delta);
        const nextY = (rotationRef.current.y + step) % 360;
  ...
  const openItemFromElement = el => {
    ...
    img.alt = rawAlt;
    img.style.width = '100%';
    img.style.height = 'calc(100% - 60px)';
    img.style.objectFit = 'cover';
    img.style.filter = grayscale ? 'grayscale(1)' : 'none';
    
    const btn = document.createElement('button');
    btn.innerHTML = 'View Details';
    btn.className = 'mt-2.5 mb-2.5 bg-brand-yellow text-brand-blue px-6 py-2 rounded-full font-black text-sm shadow-xl transition-all hover:scale-105 opacity-0 cursor-pointer pointer-events-auto';
    btn.style.transition = `opacity ${enlargeTransitionMs}ms ease`;
    btn.onclick = (e) => {
      e.stopPropagation();
          ...
          <div className="stage">
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                ...
                >
                  <div
                    className="item__image absolute block overflow-hidden cursor-pointer bg-white dark:bg-slate-900 dark:border-slate-800 transition-transform duration-300"
                    role="button"
                    tabIndex={0}
