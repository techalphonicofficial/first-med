...
export function EmptyState({
  ...
  return (
    <div className="rounded-[1.5rem] border border-dashed border-brand-blue/20 bg-white dark:bg-slate-900 dark:border-slate-800/88 p-8 text-center shadow-card">
      ...
      {tips.length ? (
        <div className="mx-auto mt-5 grid max-w-xl gap-2 sm:grid-cols-3">
          {tips.map((tip) => (
            <span key={tip} className="rounded-2xl bg-sky-50 dark:bg-slate-900 px-3 py-2 text-xs font-black text-brand-blue">
              {tip}
            </span>
          ))}
        </div>
      ...
      {(actionLabel && actionHref) || (secondaryLabel && secondaryHref) ? (
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          ...
          {secondaryLabel && secondaryHref ? <Button href={secondaryHref} className="bg-sky-50 dark:bg-slate-900 text-brand-blue hover:bg-sky-100">{secondaryLabel}</Button> : null}
        </div>
      ) : null}
    </div>
  );
