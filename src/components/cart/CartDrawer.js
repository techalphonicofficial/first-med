      ...
      {open ? (
        <motion.div className="fixed inset-0 z-[95]" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <motion.aside
            ...
            className="absolute right-0 top-0 flex h-full w-[min(420px,100vw)] flex-col bg-white dark:bg-slate-900 dark:border-slate-800 shadow-premium"
          >
            {/* Header */}
            <div className="flex items-center justify-between border-b border-sky-100 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-slate-500 dark:text-white">{cart.length} item{cart.length !== 1 ? "s" : ""}</p>
                </div>
              </div>
              <button onClick={close} className="rounded-full bg-sky-50 dark:bg-slate-900 p-2 text-brand-blue hover:bg-sky-100" aria-label="Close cart">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-auto p-5">
              {cart.length ? (
                <div className="grid gap-4">
                  {cart.map((item) => (
                    <div key={item.id} className="grid grid-cols-[72px_1fr_auto] items-center gap-4 rounded-2xl border border-slate-200 bg-white dark:bg-slate-900 dark:border-slate-800 p-3 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-card">
                      ...
                      <div className="min-w-0 self-start pt-1">
                        <Link onClick={close} href={`/products/${item.slug}/`} className="line-clamp-2 text-sm font-black leading-tight text-slate-800 dark:text-white hover:text-brand-blue">
                          {item.name}
                        </Link>
                      ...
                      <div className="flex flex-col items-center gap-1.5 rounded-full bg-sky-50 dark:bg-slate-900 p-1">
                        <button className="grid size-6 place-items-center rounded-full bg-white dark:bg-slate-900 dark:border-slate-800 text-brand-blue shadow-sm hover:bg-brand-blue hover:text-white transition" onClick={() => updateQty(item.id, item.quantity + 1)} aria-label="Increase quantity">
                          <Plus size={12} />
                        </button>
                        <span className="text-xs font-black">{item.quantity}</span>
                        <button className="grid size-6 place-items-center rounded-full bg-white dark:bg-slate-900 dark:border-slate-800 text-brand-blue shadow-sm hover:bg-brand-blue hover:text-white transition" onClick={() => updateQty(item.id, item.quantity - 1)} aria-label="Decrease quantity">
                          <Minus size={12} />
                        </button>
                      </div>
                    </div>
            ...
            {cart.length > 0 && (
              <div className="border-t border-sky-100 p-5">
                ...
                {/* Price summary */}
                <div className="grid gap-1.5 text-sm font-bold text-slate-500 dark:text-white">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <Link href="/cart/" onClick={close} className="flex min-h-[2.65rem] items-center justify-center rounded-full border border-sky-100 bg-white dark:bg-slate-900 dark:border-slate-800 px-5 py-2.5 text-sm font-black text-brand-blue shadow-card transition duration-200 hover:bg-sky-50 hover:shadow-premium active:scale-95">
                    View cart
                  </Link>
