            ...
            <div className="flex-1 overflow-auto p-5">
              {cart.length ? (
                <div className="grid gap-4">
                  {cart.map((item) => (
                    <div key={item.id} className="grid grid-cols-[72px_1fr_auto] items-center gap-3 rounded-2xl border border-sky-100 bg-sky-50/60 p-3">
                      <div className="relative size-[72px] overflow-hidden rounded-xl bg-white shadow-card">
                        <Image src={item.image} alt={item.imageAlt || item.name} fill sizes="72px" className="object-contain p-2" />
                      </div>
                      <div className="min-w-0">
                        <Link onClick={close} href={`/products/${item.slug}/`} className="line-clamp-2 text-sm font-black leading-5 hover:text-brand-blue">
                          {item.name}
                        </Link>
                        <p className="mt-1 text-sm font-black text-brand-blue">Rs. {item.price}</p>
                        {item.rxRequired && (
                          <span className="mt-1 inline-block rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-black text-amber-700">Rx</span>
                        )}
                      </div>
                      <div className="flex flex-col items-center gap-1">
                        <button className="grid h-7 w-7 place-items-center rounded-full bg-white text-brand-blue shadow-card hover:bg-sky-50" onClick={() => updateQty(item.id, item.quantity + 1)} aria-label="Increase quantity">
                          <Plus size={13} />
                        </button>
                        <span className="text-sm font-black">{item.quantity}</span>
                        <button className="grid h-7 w-7 place-items-center rounded-full bg-white text-brand-blue shadow-card hover:bg-sky-50" onClick={() => updateQty(item.id, item.quantity - 1)} aria-label="Decrease quantity">
                          <Minus size={13} />
                        </button>
                      </div>
                    </div>
            ...
            {cart.length > 0 && (
              <div className="border-t border-sky-100 p-5">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3">
                  <Button href="/cart/" className="bg-white text-brand-blue hover:bg-sky-50 shadow-card" onClick={close}>View cart</Button>
