            ...
            <div className="grid gap-6 lg:grid-cols-[1.25fr_0.75fr]">
              <div>
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  <button className="inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-black text-brand-blue shadow-card transition hover:-translate-y-0.5">
                    {module.primaryAction} <ArrowRight size={15} />
                  </button>
                  <button onClick={downloadCsv} className="inline-flex items-center gap-2 rounded-full bg-white/10 px-5 py-2.5 text-sm font-black text-white backdrop-blur transition hover:bg-white/20">
              ...
              <div className="grid gap-3 rounded-2xl bg-white/10 p-4">
                {module.workflows.slice(0, 5).map((item) => (
                  <div key={item} className="flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2.5 text-sm font-black">
            ...
            <div className="soft-card overflow-hidden rounded-2xl">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-sky-100 bg-sky-50/50 px-5 py-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-52 rounded-full border border-sky-100 bg-white py-2 pl-9 pr-3 text-xs font-bold outline-brand-blue"
                  />
                </div>
              </div>
              ...
              {selectedState === "Form" ? (
                <div className="grid gap-5 p-5 lg:grid-cols-[1fr_280px]">
                  <form className="grid gap-4">
                    ...
                    >
                      ...
                      <button type="button" className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-5 py-2.5 text-sm font-black text-brand-blue">
                ...
                <div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px]">
                  <label className="grid min-h-64 cursor-pointer place-items-center rounded-[2rem] border-2 border-dashed border-brand-blue/20 bg-sky-50/70 p-8 text-center transition hover:border-brand-blue/50 hover:bg-sky-50">
                    <input
                      type="file"
                      className="hidden"
                      onChange={(event) => {
                    ...
                    />
                    <div>
                      <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-white text-brand-blue shadow-card">
                    ...
                    {uploadResult ? (
                      <div className="mt-4 grid gap-3">
                        {[
                          ...
                          <div key={label} className="flex items-center justify-between rounded-xl bg-sky-50 px-3 py-2 text-sm font-black">
                      ...
                      <p className="mt-3 text-sm font-semibold leading-6 text-slate-500">No file selected yet.</p>
                    )}
                  </div>
                </div>
              ) : selectedState === "Map" ? (
                <div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px]">
                  <div className="relative min-h-96 overflow-hidden rounded-[2rem] bg-[#e6f4f1]">
                    ...
                    <div className="absolute bottom-5 left-5 rounded-2xl bg-white p-4 shadow-card">
                      <p className="text-sm font-black text-brand-blue">{map.title}</p>
                      <p className="mt-1 text-xs font-semibold text-slate-500">{map.start} to {map.end}</p>
                    </div>
                  </div>
                  <div className="soft-card rounded-2xl p-5">
                    <h3 className="text-lg font-black">Map simulation</h3>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Provider-ready map UI for address serviceability, delivery tracking, warehouse coverage, and route previews.</p>
                ...
                <div className="grid gap-5 p-5 lg:grid-cols-[1fr_320px]">
                  <div className="rounded-[2rem] bg-brand-navy p-8 text-white shadow-premium">
                    <div className="grid h-14 w-14 place-items-center rounded-3xl bg-white/10 text-brand-yellow">
                      <WalletCards size={28} />
                    </div>
                    ...
                    <button
                      ...
                      className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-5 py-2.5 text-sm font-black text-brand-blue"
                    >
                ...
                <div className="p-8 text-center">
                  <div className="mx-auto grid h-16 w-16 place-items-center rounded-3xl bg-sky-50 text-brand-blue">
                    <FileText size={28} />
                  </div>
                  <h3 className="mt-4 text-xl font-black">No {active.label.toLowerCase()} records yet</h3>
                  <p className="mx-auto mt-2 max-w-md text-sm font-semibold leading-6 text-slate-500">Once records exist, this area will show the queue, filters, actions, and audit context for this feature.</p>
                </div>
              ) : selectedState === "Error" ? (
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    {recordsQuery.isLoading ? (
                      <tr>
                        <td colSpan={5} className="px-5 py-8 text-center text-sm font-black text-slate-400">Loading records...</td>
                      </tr>
                    ) : null}
                  </tbody>
                </table>
