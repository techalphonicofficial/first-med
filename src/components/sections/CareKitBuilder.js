...
export function CareKitBuilder() {
  ...
  return (
    <section className="mx-auto max-w-[104rem] px-4 py-10 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
      <div className="rounded-[2rem] border border-white/80 bg-white dark:bg-slate-900 dark:border-slate-800/88 p-5 shadow-premium backdrop-blur-xl md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div>
            ...
            {activeKit && (
              <p className="mt-2 text-sm font-semibold text-slate-500 dark:text-white">{activeKit.emoji} {activeKit.desc}</p>
            )}
          </div>
        ...
        <div className="mt-7 grid gap-4 md:grid-cols-3">
          {items.map((product) => (
            <div key={product.id} className="soft-card group rounded-2xl p-4 transition duration-200 hover:-translate-y-1 hover:shadow-premium">
              {/* Product image */}
              <div className="relative mx-auto mb-4 aspect-square w-full max-w-[100px] overflow-hidden rounded-2xl bg-sky-50 dark:bg-slate-900">
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold text-slate-400 dark:text-white">{product.brand}</p>
                </div>
                <Badge variant={product.rxRequired ? "yellow" : "green"}>{product.rxRequired ? "Rx" : "OTC"}</Badge>
              </div>

              <p className="mt-2 line-clamp-2 text-xs font-semibold leading-5 text-slate-500 dark:text-white">{product.usage}</p>

              <div className="mt-3 flex items-end justify-between">
                <span className="text-sm font-black text-slate-900 dark:text-white">Rs. {product.price}</span>
                <span className="text-xs font-bold text-slate-400 dark:text-white line-through">Rs. {product.mrp}</span>
              </div>
