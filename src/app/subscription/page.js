    ...
    <div className="mx-auto max-w-[104rem] px-4 py-8 pb-28 sm:px-6 lg:px-8 xl:px-10">
      <div className="mb-8 overflow-hidden rounded-[2rem] bg-brand-navy p-7 text-white shadow-premium sm:p-9">
        <div className="grid gap-6 lg:grid-cols-[1fr_auto]">
          ...
          <div className="flex flex-col gap-3 justify-center">
            <Link href="/products" className="inline-flex items-center justify-center gap-2 rounded-full bg-brand-yellow px-6 py-3 text-sm font-black text-slate-900 shadow-glow transition hover:-translate-y-0.5">
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_350px]">
        ...
        <div className="grid gap-6">
          <h2 className="text-xl font-black text-slate-900">Current Plans</h2>
            ...
            {subscriptions.map((sub) => (
              <div key={sub.id} className="soft-card flex flex-col sm:flex-row sm:items-center justify-between gap-6 rounded-2xl p-6 transition hover:shadow-premium hover:-translate-y-1">
                ...
                <div className="flex flex-wrap sm:flex-col gap-2 shrink-0">
                  {sub.status === 'Active' ? (
                    <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-amber-50 text-amber-700 px-4 py-2.5 text-sm font-black hover:bg-amber-100 transition">
                      <Pause size={16} /> Pause Plan
                    </button>
                  ) : (
                    <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-50 text-emerald-700 px-4 py-2.5 text-sm font-black hover:bg-emerald-100 transition">
                      <Play size={16} /> Resume Plan
                    </button>
                  )}
                  <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-sky-100 bg-white text-brand-blue px-4 py-2.5 text-sm font-black hover:bg-sky-50 transition shadow-sm">
        ...
        <div className="grid gap-4 h-fit sticky top-8">
          <div className="soft-card rounded-2xl p-6">
            <h2 className="text-lg font-black mb-4 text-slate-900">Subscription Benefits</h2>
            <ul className="text-sm font-semibold text-slate-600 space-y-4">
              <li className="flex gap-3">
                <CheckCircle2 size={18} className="text-emerald-500 shrink-0" />
                <span><strong className="text-slate-800 font-bold block">15% Guaranteed Discount</strong> On all subscribed medicines.</span>
              </li>
              <li className="flex gap-3">
                <CheckCircle2 size={18} className="text-emerald-500 shrink-0" />
                <span><strong className="text-slate-800 font-bold block">Free Shipping</strong> Delivery charges waived on recurring orders.</span>
              </li>
              <li className="flex gap-3">
                <CheckCircle2 size={18} className="text-emerald-500 shrink-0" />
                <span><strong className="text-slate-800 font-bold block">Flexible Controls</strong> Skip a month or pause anytime with zero penalty.</span>
              </li>
            </ul>
          </div>
          ...
          <div className="soft-card rounded-2xl p-6 bg-slate-50 border border-slate-100">
            ...
            <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex w-full items-center justify-center gap-2 rounded-xl bg-white text-rose-600 border border-rose-100 px-4 py-2.5 text-xs font-black hover:bg-rose-50 transition">
