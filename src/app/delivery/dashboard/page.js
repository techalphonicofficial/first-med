    ...
    <div className="mx-auto max-w-[104rem] px-4 py-8 pb-28 sm:px-6 lg:px-8 xl:px-10">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">Ravi Kumar · DL-04-AB-1234</p>
        </div>
        <button
          onClick={() => setOnline(!online)}
          className={`flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-black transition ${online ? "bg-emerald-500 text-white shadow-glow" : "bg-slate-200 text-slate-500"}`}
        >
        ...
        <div className="grid gap-4">
          ...
          {online ? deliveries.map((del) => (
            <div key={del.id} className="soft-card rounded-2xl p-5 transition hover:-translate-y-1 hover:shadow-premium">
              <div className="flex items-center justify-between">
                <span className="rounded-full bg-sky-50 px-2.5 py-1 text-xs font-black text-brand-blue">{del.id}</span>
