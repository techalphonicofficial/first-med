  ...
  return (
    <div className="mx-auto max-w-[104rem] px-4 py-8 pb-28 sm:px-6 lg:px-8 xl:px-10">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <button onClick={(e) => { e.preventDefault(); toast.info('Preparing statement for download...'); }} className="flex items-center gap-2 rounded-full bg-white dark:bg-slate-900 dark:border-slate-800 px-5 py-2.5 text-sm font-black text-emerald-600 shadow-card hover:bg-emerald-50 transition">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mb-6">
        ...
        <div className="soft-card rounded-2xl p-6">
          <div className="flex items-center gap-3 mb-4 text-slate-500 dark:text-white">
            <IndianRupee size={24} />
            <h2 className="text-sm font-black uppercase tracking-widest">Total Earned (This Week)</h2>
          </div>
          <p className="text-4xl font-black text-slate-900 dark:text-white mb-2">Rs. 3,240</p>
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={log.date} className="flex items-center justify-between border-b border-emerald-50 pb-4 last:border-0 last:pb-0">
              <div>
                <p className={`font-black ${log.active ? 'text-emerald-600' : 'text-slate-800 dark:text-white'}`}>{log.day}</p>
                <p className="text-xs font-semibold text-slate-400 dark:text-white">{log.date} • {log.trips} completed trips</p>
              </div>
              <p className="text-lg font-black text-slate-900 dark:text-white">{log.amount}</p>
            </div>
          ))}
        </div>
      </div>
