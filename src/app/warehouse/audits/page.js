    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-slate-900 dark:text-white">Stock Audits & Damage Logs</h1>
        </div>
        <div className="flex gap-3">
          <button onClick={(e) => { e.preventDefault(); toast.info('Opening damage report form...'); }} className="flex items-center gap-2 rounded-full bg-white dark:bg-slate-900 dark:border-slate-800 border border-rose-100 text-rose-600 px-5 py-2.5 text-sm font-black shadow-sm hover:bg-rose-50 transition">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-sky-50 bg-slate-50 dark:bg-slate-950 dark:border-slate-800 text-left">
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Audit ID</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Type</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Variance</th>
            ...
            <tbody>
              {mockAudits.map((audit) => (
                <tr key={audit.id} className="border-b border-sky-50 last:border-0 hover:bg-sky-50 dark:bg-slate-900/40 transition">
                  <td className="px-5 py-4">
                    <p className="font-black text-brand-blue">{audit.id}</p>
                    <p className="text-xs font-semibold text-slate-400 dark:text-white">{audit.date} • By {audit.operator}</p>
                  </td>
                  <td className="px-5 py-4 font-bold text-slate-700 dark:text-white">{audit.type}</td>
                  <td className="px-5 py-4">
                    <span className={`font-black ${audit.variance.startsWith('-') ? 'text-rose-600' : audit.variance.startsWith('+') ? 'text-emerald-600' : 'text-slate-500 dark:text-white'}`}>
                      {audit.variance}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    <button onClick={(e) => { e.preventDefault(); toast.info('Generating audit report PDF...'); }} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600 dark:text-white hover:bg-slate-200 transition">
