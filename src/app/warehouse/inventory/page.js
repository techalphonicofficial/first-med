    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-slate-900 dark:text-white">Inventory Management</h1>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-sky-50 bg-slate-50 dark:bg-slate-950 dark:border-slate-800 text-left">
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Item Details</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Batch & Expiry</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Location</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Stock Level</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white text-right">Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((item) => (
                <tr key={item.id} className="border-b border-sky-50 last:border-0 hover:bg-sky-50 dark:bg-slate-900/40 transition">
                  <td className="px-5 py-4">
                    <p className="font-black text-brand-blue">{item.name}</p>
                    <p className="text-xs font-semibold text-slate-400 dark:text-white">SKU: {item.id}</p>
                  </td>
                  <td className="px-5 py-4">
                    <p className="font-bold text-slate-700 dark:text-white">{item.batch}</p>
                    <p className="text-xs font-semibold text-slate-500 dark:text-white">Exp: {item.expiry}</p>
                  </td>
                  <td className="px-5 py-4">
                    <span className="rounded bg-sky-50 dark:bg-slate-900 px-2 py-1 text-xs font-black text-brand-blue border dark:border-slate-800 border dark:border-slate-800-sky-100">
                      {item.location}
                    </span>
                  </td>
                  <td className="px-5 py-4">
                    <p className="font-black text-slate-900 dark:text-white text-lg">{item.quantity} <span className="text-xs font-bold text-slate-500 dark:text-white">units</span></p>
                  ...
                  <td className="px-5 py-4 text-right">
                    <button onClick={(e) => { e.preventDefault(); toast.info('Opening stock adjustment panel...'); }} className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-600 dark:text-white hover:bg-slate-200 transition">
