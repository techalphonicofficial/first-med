    ...
    <div className="grid gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="soft-card rounded-2xl p-5 border-l-4 border-amber-400">
          <p className="text-sm font-black text-slate-500">Pending Reviews</p>
          <p className="mt-2 text-3xl font-black text-amber-600">2</p>
          <p className="mt-1 text-xs font-semibold text-slate-400">Require immediate action</p>
        </div>
        <div className="soft-card rounded-2xl p-5 border-l-4 border-emerald-500">
          <p className="text-sm font-black text-slate-500">Active Vendors</p>
          <p className="mt-2 text-3xl font-black text-emerald-600">342</p>
          <p className="mt-1 text-xs font-semibold text-slate-400">Fully verified</p>
        </div>
        <div className="soft-card rounded-2xl p-5 border-l-4 border-rose-500">
          <p className="text-sm font-black text-slate-500">Rejected</p>
          <p className="mt-2 text-3xl font-black text-rose-600">14</p>
          <p className="mt-1 text-xs font-semibold text-slate-400">Failed compliance</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-sky-50 bg-slate-50 text-left">
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400">Vendor</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400">Owner</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400">Documents</th>
            ...
            <tbody>
              {filtered.map((vendor) => (
                <tr key={vendor.id} className="border-b border-sky-50 last:border-0 hover:bg-sky-50/40 transition">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 place-items-center rounded-xl bg-sky-50 text-brand-blue">
                        <Building2 size={20} />
                      </div>
                      <div>
                        <p className="font-black text-brand-blue">{vendor.name}</p>
                        <p className="text-xs font-semibold text-slate-400">{vendor.id} • Applied {vendor.applied}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    {vendor.status === 'Pending' ? (
                      ...
                      <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-4 py-1.5 text-xs font-black text-brand-blue hover:bg-sky-100 transition">
