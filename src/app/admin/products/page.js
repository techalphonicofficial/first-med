    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-slate-900 dark:text-white">Global Product Catalog</h1>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={(e) => { e.preventDefault(); toast.info('Opening advanced filters...'); }} className="flex items-center gap-2 rounded-full bg-white dark:bg-slate-900 dark:border-slate-800 px-4 py-2.5 text-sm font-black text-brand-blue border border-sky-100 shadow-sm hover:bg-sky-50 transition">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-sky-50 bg-slate-50 dark:bg-slate-950 dark:border-slate-800 text-left">
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Product</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Category</th>
                <th className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-400 dark:text-white">Price & Stock</th>
            ...
            <tbody>
              {filtered.map((product) => (
                <tr key={product.id} className="border-b border-sky-50 last:border-0 hover:bg-sky-50 dark:bg-slate-900/40 transition">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-sky-50 dark:bg-slate-900 text-brand-blue">
                        <Package size={20} />
                      </div>
                      <div>
                        <p className="font-black text-brand-blue">{product.name}</p>
                        <p className="text-xs font-semibold text-slate-400 dark:text-white">{product.id} • {product.vendor}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-5 py-4 font-bold text-slate-700 dark:text-white">{product.category}</td>
                  <td className="px-5 py-4">
                  ...
                  <td className="px-5 py-4 text-right">
                    {product.status === 'Pending' ? (
                      ...
                      <button onClick={() => openEdit(product)} className="rounded-full bg-slate-100 px-4 py-1.5 text-xs font-black text-slate-600 dark:text-white hover:bg-slate-200 transition">
                        Edit
                      </button>
                    )}
                  </td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Edit Product details">
        <form onSubmit={saveProduct} className="grid gap-4">
          <label className="grid gap-2 text-sm font-black">
            Product Name
            <input name="name" required defaultValue={editingProduct?.name || ""} className="rounded-2xl border dark:border-slate-800 border dark:border-slate-800-sky-100 bg-sky-50 dark:bg-slate-900 px-4 py-3 font-semibold outline-brand-blue" />
          </label>
          <label className="grid gap-2 text-sm font-black">
            Category
            <input name="category" required defaultValue={editingProduct?.category || ""} className="rounded-2xl border dark:border-slate-800 border dark:border-slate-800-sky-100 bg-sky-50 dark:bg-slate-900 px-4 py-3 font-semibold outline-brand-blue" />
          </label>
          <div className="grid grid-cols-2 gap-4">
            <label className="grid gap-2 text-sm font-black">
              Price
              <input name="price" required defaultValue={editingProduct?.price || ""} className="rounded-2xl border dark:border-slate-800 border dark:border-slate-800-sky-100 bg-sky-50 dark:bg-slate-900 px-4 py-3 font-semibold outline-brand-blue" />
            </label>
            <label className="grid gap-2 text-sm font-black">
              Stock Status
              <select name="stock" required defaultValue={editingProduct?.stock || "High"} className="rounded-2xl border dark:border-slate-800 border dark:border-slate-800-sky-100 bg-sky-50 dark:bg-slate-900 px-4 py-3 font-semibold outline-brand-blue">
