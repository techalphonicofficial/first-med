    ...
    <div className="grid gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-black text-slate-900">Prescription Verification</h1>
        </div>
        <div className="rounded-full bg-rose-50 border border-rose-100 px-4 py-2 flex items-center gap-2">
          <AlertTriangle size={16} className="text-rose-600" />
          <span className="text-sm font-black text-rose-800">2 High Priority Pending</span>
        </div>
      </div>
          ...
          {mockPrescriptions.map((prx) => (
            <div key={prx.id} className="soft-card rounded-2xl p-5 border-l-4 border-l-brand-blue transition hover:shadow-premium hover:-translate-y-1">
                ...
                <div className="flex gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-lg font-black text-slate-900">{prx.id}</h3>
                      {prx.priority === 'High' && (
                        <span className="rounded bg-rose-100 px-1.5 py-0.5 text-[10px] font-black uppercase text-rose-700">Urgent SLA</span>
                      )}
                    </div>
                    <p className="text-sm font-bold text-slate-600">{prx.patient}</p>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3">
                <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-500 py-2.5 text-sm font-black text-white hover:bg-emerald-600 transition shadow-glow">
                  <Check size={16} /> Verify & Approve
                </button>
                <button onClick={(e) => { e.preventDefault(); toast.success('Action completed successfully!'); }} className="flex items-center justify-center gap-2 rounded-xl bg-rose-50 text-rose-700 py-2.5 text-sm font-black hover:bg-rose-100 transition">
                  <X size={16} /> Reject Rx
                </button>
              </div>
            </div>
