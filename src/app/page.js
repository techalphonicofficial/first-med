              ...
              {categories.slice(0, 6).map((category) => {
                ...
                return (
                  <a
                    ...
                    className="soft-card group rounded-[1.5rem] p-4 transition duration-300 hover:-translate-y-1 hover:shadow-premium"
                  >
                    ...
                    <p className="text-xs font-black leading-tight">{category}</p>
                  </a>
                );
              })}
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {needKits.map(({ name, desc, items, href, color, accent, icon: Icon }, index) => (
              <Reveal key={name} delay={index * 0.08}>
                <a
                  href={href}
                  className={`group soft-card block rounded-2xl bg-gradient-to-br ${color} p-6 transition duration-300 hover:-translate-y-1 hover:shadow-premium`}
                >
                  <div className="flex items-center justify-between">
                    <div className={`grid h-10 w-10 place-items-center rounded-2xl ${accent} text-white`}>
                      <Icon size={18} />
                    </div>
                    <span className="rounded-full bg-white/80 px-3 py-1 text-xs font-black text-slate-600 shadow-card">
                      {items.length} items
                    </span>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    {items.map((item) => (
                      <span key={item} className="rounded-full bg-white/70 px-2 py-0.5 text-[10px] font-black text-slate-600">
                        {item}
                      </span>
                    ))}
                  </div>
                  <div className="mt-4 flex items-center gap-1 text-xs font-black text-slate-600 group-hover:text-brand-blue">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {vendors.map((vendor, index) => (
            <Reveal key={vendor.name} delay={index * 0.08}>
              <div className="soft-card group rounded-2xl p-5 transition duration-300 hover:-translate-y-1 hover:shadow-premium">
                ...
                <h3 className="mt-3 font-black">{vendor.name}</h3>
        ...
        <Reveal>
          <div className="grid gap-6 rounded-2xl bg-brand-navy p-6 text-white shadow-soft md:grid-cols-[0.85fr_1fr] md:p-9">
            <div>
              ...
              <Button href="/prescription" className="mt-6 bg-white text-brand-blue hover:bg-sky-50">
                Upload prescription
              </Button>
            </div>
